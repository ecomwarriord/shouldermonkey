...
const MortgageBrokerPlayer = dynamic(
  () => import('../../components/MortgageBrokerPlayer').then(m => ({ default: m.MortgageBrokerPlayer })),
  { ssr: false, loading: () => <div style={{ position: 'absolute', inset: 0, background: '#080d1a' }} /> }
)
    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: 64, overflow: 'visible' }}>
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
          ...
          <div style={{ position: 'absolute', top: '20%', right: '5%', width: 500, height: 500, borderRadius: '50%', background: 'rgba(212,160,74,0.06)', filter: 'blur(80px)' }} />
          <div style={{ position: 'absolute', bottom: '10%', left: '15%', width: 400, height: 400, borderRadius: '50%', background: 'rgba(212,160,74,0.04)', filter: 'blur(80px)' }} />
        </div>
        ...
        <div style={{ ...W, position: 'relative', display: 'grid', gridTemplateColumns: '5fr 4fr', gap: 'clamp(3rem, 5vw, 6rem)', alignItems: 'center', padding: `clamp(5rem, 10vw, 8rem) 0`, maxWidth: 1440 }}>
          ...
          <div style={{ paddingLeft: 'clamp(1.5rem, 5vw, 8rem)', paddingRight: 'clamp(2rem, 4vw, 5rem)' }}>
            ...
            <h1 style={{ fontFamily: playfair.style.fontFamily, fontSize: 'clamp(54px, 7vw, 104px)', fontWeight: 800, color: C.text, lineHeight: 0.92, letterSpacing: '-0.02em', marginBottom: 20, marginLeft: 'clamp(-1rem, -2.5vw, -3rem)' }}>
              <motion.span initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.08, ease: [0.22,1,0.36,1] }} style={{ display: 'block' }}>Your home</motion.span>
              <motion.span initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.20, ease: [0.22,1,0.36,1] }} style={{ display: 'block', color: C.accent }}>starts here.</motion.span>
            </h1>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.30, ease: [0.22,1,0.36,1] }}
              ...
              More enquiries converted. Pre-approvals in 24 hours. Your pipeline always full.
            </motion.p>
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.38, ease: [0.22,1,0.36,1] }}
              ...
              Your brokerage could look exactly like this — and we&apos;ll have it live in 7 days.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.5 }}
              ...
              {[
                { stat: '4.9', label: '1,240 Google reviews' },
                { stat: '$2.8B', label: 'loans settled' },
                { stat: '40+', label: 'lender panel' },
              ].map((item, i) => (
      ...
      {/* ── Signature stat ── */}
      <section style={{ padding: 'clamp(5rem, 9vw, 8rem) 0 clamp(4rem, 8vw, 7rem)', background: '#03060f', overflow: 'hidden', position: 'relative', zIndex: 2, marginTop: '-80px', borderRadius: '28px 28px 0 0' }}>
      ...
      <section style={{ overflow: 'hidden', background: C.surface, position: 'relative', maxHeight: '65vh', minHeight: 320 }}>
        <video src="/videos/mortgage-broker-hero.mp4" autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', maxHeight: '65vh', minHeight: 320 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(8,13,26,0.2) 0%, transparent 30%, transparent 65%, rgba(8,13,26,0.5) 100%)', pointerEvents: 'none' }} />
      </section>
          ...
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.accentLt, border: `1px solid ${C.border}`, borderRadius: 100, padding: '6px 16px', marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.accent }}>The Atlas Experience</span>
              </div>
              <h2 style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.4rem)', fontWeight: 800, color: C.text, margin: '0 0 12px' }}>See the system in action</h2>
              <p style={{ fontSize: 16, color: C.muted, maxWidth: 480, margin: '0 auto' }}>From first enquiry to settlement — every client touchpoint handled.</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <div style={{ position: 'relative', maxWidth: 960, margin: '0 auto', borderRadius: 24, overflow: 'hidden', background: C.surface, aspectRatio: '16/9', boxShadow: `0 0 0 1px ${C.border}, 0 40px 80px rgba(0,0,0,0.25)` }}>
              <div style={{ position: 'absolute', inset: 0 }}>
                <MortgageBrokerPlayer />
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <AnimatePresence>
        {stickyVisible && (
          <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
            ...
            style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 300, display: 'flex', alignItems: 'center', gap: 14, background: C.surface, borderRadius: 100, padding: '12px 14px 12px 22px', boxShadow: '0 16px 50px rgba(0,0,0,0.3)', whiteSpace: 'nowrap', border: `1px solid ${C.border}` }}>
