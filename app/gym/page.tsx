...
const GymPlayer = dynamic(
  () => import('../../components/GymPlayer').then(m => ({ default: m.GymPlayer })),
  { ssr: false, loading: () => <div style={{ position: 'absolute', inset: 0, background: '#07090a' }} /> }
)
...
export default function GymPage() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      <style>{`::selection{background:${C.accent};color:#000}*{cursor:auto!important}button,a{cursor:pointer!important}h2{font-family:${spaceGrotesk.style.fontFamily};letter-spacing:-0.04em}`}</style>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: 64, overflow: 'visible' }}>
        ...
        <div style={{ width: '100%', maxWidth: 1440, margin: '0 auto', padding: `clamp(5rem, 10vw, 8rem) 0`, position: 'relative' }}>
          ...
          <div style={{ paddingLeft: 'clamp(1.5rem, 5vw, 8rem)', paddingRight: 'clamp(2rem, 4vw, 5rem)' }}>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.34, ease: [0.22,1,0.36,1] }}
              style={{ color: C.text, fontSize: 18, lineHeight: 1.6, fontWeight: 600, marginBottom: 8, maxWidth: 420 }}>
              More trials converted. More members retained. Running without you lifting a finger.
            </motion.p>
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.40, ease: [0.22,1,0.36,1] }}
              style={{ fontSize: 14, lineHeight: 1.6, color: C.muted, margin: '0 0 32px', maxWidth: 420 }}>
              Your gym could look exactly like this — and we&apos;ll have it live in 7 days.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.5 }}
              ...
              {[
                { stat: '4.9', label: '612 Google reviews' },
                { stat: '340+', label: 'active members' },
                { stat: '8', label: 'certified coaches' },
              ].map((item, i) => (
      ...
      {/* ── Signature stat ── */}
      <section style={{ padding: 'clamp(5rem, 9vw, 8rem) 0 clamp(4rem, 8vw, 7rem)', background: '#030a04', overflow: 'hidden', position: 'relative', zIndex: 2, marginTop: '-80px', borderRadius: '28px 28px 0 0' }}>
        <div style={{ position: 'absolute', right: '-3%', top: '50%', transform: 'translateY(-50%)', fontSize: 'clamp(200px, 35vw, 400px)', fontWeight: 900, color: 'rgba(34,197,94,0.04)', fontFamily: spaceGrotesk.style.fontFamily, lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>73</div>
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 clamp(1.5rem, 4vw, 3rem)', display: 'flex', alignItems: 'flex-end', gap: 'clamp(2rem, 5vw, 5rem)', flexWrap: 'wrap' }}>
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.22,1,0.36,1] }}>
            <div style={{ fontFamily: spaceGrotesk.style.fontFamily, fontSize: 'clamp(80px, 16vw, 180px)', fontWeight: 900, color: C.accent, letterSpacing: '-0.05em', lineHeight: 0.85 }}>73%</div>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(240,255,244,0.3)', marginTop: 16 }}>Trial-to-member conversion</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2, ease: [0.22,1,0.36,1] }}
            ...
            <div style={{ fontSize: 'clamp(20px, 2.5vw, 32px)', fontWeight: 700, color: 'rgba(240,255,244,0.9)', letterSpacing: '-0.02em', lineHeight: 1.3, marginBottom: 16, fontFamily: spaceGrotesk.style.fontFamily }}>Industry average is 45%. Apex converts at 73% — because no lead goes unanswered.</div>
            <div style={{ fontSize: 14, color: 'rgba(240,255,244,0.35)', lineHeight: 1.65 }}>Every missed call, every DM, every form submission auto-replied in under 90 seconds.</div>
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ overflow: 'hidden', background: C.surface, position: 'relative', maxHeight: '65vh', minHeight: 320 }}>
        <video src="/videos/gym-hero.mp4" autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', maxHeight: '65vh', minHeight: 320 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(7,9,10,0.2) 0%, transparent 30%, transparent 65%, rgba(7,9,10,0.55) 100%)', pointerEvents: 'none' }} />
      </section>
          ...
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.accentLt, border: `1px solid ${C.border}`, borderRadius: 100, padding: '6px 16px', marginBottom: 16 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.accent, display: 'inline-block', animation: 'pulse 2s infinite' }} />
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.accent }}>The Apex Experience</span>
              </div>
              <h2 style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.4rem)', fontWeight: 900, color: C.text, margin: '0 0 12px' }}>See the system in action</h2>
              <p style={{ fontSize: 16, color: C.muted, maxWidth: 480, margin: '0 auto' }}>From first DM to loyal member — every touchpoint automated.</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <div style={{ position: 'relative', maxWidth: 960, margin: '0 auto', borderRadius: 24, overflow: 'hidden', background: C.bg, aspectRatio: '16/9', boxShadow: `0 0 0 1px ${C.border}, 0 40px 80px rgba(0,0,0,0.3)` }}>
      ...
      <AnimatePresence>
        {stickyVisible && (
          <motion.div
            ...
            style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 300, display: 'flex', alignItems: 'center', gap: 14, background: '#0d1210', borderRadius: 100, padding: '12px 14px 12px 22px', boxShadow: '0 16px 50px rgba(0,0,0,0.4)', whiteSpace: 'nowrap', border: `1px solid ${C.border}` }}>
            <span style={{ fontSize: 14, color: C.muted, fontWeight: 500 }}>Like what you see?</span>
            <button onClick={() => { const el = document.getElementById('enquiry'); if (el) el.scrollIntoView({ behavior: 'smooth' }) }}
              style={{ background: C.accent, color: '#000', border: 'none', borderRadius: 100, padding: '10px 20px', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>
