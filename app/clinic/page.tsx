...
const ClinicPlayer = dynamic(
  () => import('../../components/ClinicPlayer').then(m => ({ default: m.ClinicPlayer })),
  { ssr: false, loading: () => <div style={{ position: 'absolute', inset: 0, background: '#f8faff' }} /> }
)
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <nav style={{
        ...
        {navOpen && (
          <div style={{ background: 'rgba(248,250,255,0.98)', borderTop: `1px solid ${C.border}`, padding: '1.5rem clamp(1.5rem, 4vw, 3rem)' }}>
            {['Our Doctors', 'Services', 'Pathology', 'Contact'].map(l => (
            ...
            <button style={{ marginTop: 20, background: C.accent, color: '#fff', border: 'none', borderRadius: 10, padding: '14px 28px', fontSize: 15, fontWeight: 700, cursor: 'pointer', width: '100%' }}>Book Appointment</button>
          </div>
        )}
      </nav>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: 64, overflow: 'visible' }}>
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
          ...
          <div style={{ position: 'absolute', top: '-20%', right: '-10%', width: 600, height: 600, borderRadius: '50%', background: 'rgba(2,132,199,0.06)', filter: 'blur(80px)' }} />
          <div style={{ position: 'absolute', bottom: '-10%', left: '10%', width: 400, height: 400, borderRadius: '50%', background: 'rgba(2,132,199,0.05)', filter: 'blur(80px)' }} />
        </div>

        <div style={{ ...W, position: 'relative', display: 'grid', gridTemplateColumns: '5fr 4fr', gap: 'clamp(3rem, 5vw, 6rem)', alignItems: 'center', padding: `clamp(5rem, 10vw, 8rem) 0`, maxWidth: 1440 }}>
          ...
          <div style={{ paddingLeft: 'clamp(1.5rem, 5vw, 8rem)', paddingRight: 'clamp(2rem, 4vw, 5rem)' }}>
            ...
            <h1 style={{ fontFamily: plusJakarta.style.fontFamily, fontSize: 'clamp(54px, 7vw, 104px)', fontWeight: 800, color: C.text, lineHeight: 0.92, letterSpacing: '-0.03em', marginBottom: 20, marginLeft: 'clamp(-1rem, -2.5vw, -3rem)' }}>
              <motion.span initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.08, ease: [0.22,1,0.36,1] }} style={{ display: 'block' }}>Pinnacle Health</motion.span>
              <motion.span initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.20, ease: [0.22,1,0.36,1] }} style={{ display: 'block', color: C.accent }}>North Sydney</motion.span>
            </h1>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.30, ease: [0.22,1,0.36,1] }}
              ...
              More patients booked. Better follow-up. Your practice running at its best.
            </motion.p>
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.38, ease: [0.22,1,0.36,1] }}
              ...
              Your clinic could look exactly like this — and we&apos;ll have it live in 7 days.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.5 }}
              ...
              {[
                { stat: '4.9', label: '847 Google reviews' },
                { stat: '14', label: 'specialist GPs' },
                { stat: '23yr', label: 'serving North Sydney' },
              ].map((item, i) => (
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.3 }} style={{ transform: 'translateX(4vw)', paddingRight: 0 }}>
      ...
      <section style={{ padding: 'clamp(5rem, 9vw, 8rem) 0 clamp(4rem, 8vw, 7rem)', background: '#040d1a', overflow: 'hidden', position: 'relative', zIndex: 2, marginTop: '-80px', borderRadius: '28px 28px 0 0' }}>
        ...
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 clamp(1.5rem, 4vw, 3rem)', display: 'flex', alignItems: 'flex-end', gap: 'clamp(2rem, 5vw, 5rem)', flexWrap: 'wrap' }}>
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.22,1,0.36,1] }}>
            <div style={{ fontFamily: plusJakarta.style.fontFamily, fontSize: 'clamp(80px, 16vw, 180px)', fontWeight: 800, color: C.accent, letterSpacing: '-0.05em', lineHeight: 0.85 }}>847</div>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(240,244,255,0.3)', marginTop: 16 }}>Five-star Google reviews</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2, ease: [0.22,1,0.36,1] }}
            ...
            <div style={{ fontSize: 'clamp(20px, 2.5vw, 32px)', fontWeight: 800, color: 'rgba(240,244,255,0.9)', letterSpacing: '-0.02em', lineHeight: 1.3, marginBottom: 16, fontFamily: plusJakarta.style.fontFamily }}>23 years serving North Sydney. The reputation is built — the system keeps it growing.</div>
            <div style={{ fontSize: 14, color: 'rgba(240,244,255,0.35)', lineHeight: 1.65 }}>Automated review requests after every visit. No awkward asks. Just consistent 5-star results.</div>
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ overflow: 'hidden', background: C.surface, position: 'relative', maxHeight: '65vh', minHeight: 320 }}>
        <video src="/videos/clinic-hero.mp4" autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', maxHeight: '65vh', minHeight: 320 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(15,24,41,0.1) 0%, transparent 30%, transparent 65%, rgba(15,24,41,0.35) 100%)', pointerEvents: 'none' }} />
      </section>
          ...
          <Reveal>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.accentLt, border: `1px solid ${C.border}`, borderRadius: 100, padding: '6px 16px', marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.accent }}>The Pinnacle Experience</span>
              </div>
              <h2 style={{ fontSize: 'clamp(1.8rem, 3.5vw, 2.4rem)', fontWeight: 800, color: C.text, margin: '0 0 12px' }}>See the system in action</h2>
              <p style={{ fontSize: 16, color: C.muted, maxWidth: 480, margin: '0 auto' }}>From first enquiry to follow-up care — every touchpoint automated.</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <div style={{ position: 'relative', maxWidth: 960, margin: '0 auto', borderRadius: 24, overflow: 'hidden', background: C.surface, aspectRatio: '16/9', boxShadow: `0 0 0 1px ${C.border}, 0 40px 80px rgba(2,132,199,0.08)` }}>
      ...
      <AnimatePresence>
        {stickyVisible && (
          <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
            ...
            style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 300, display: 'flex', alignItems: 'center', gap: 14, background: C.text, borderRadius: 100, padding: '12px 14px 12px 22px', boxShadow: '0 16px 50px rgba(2,132,199,0.2)', whiteSpace: 'nowrap', border: `1px solid ${C.border}` }}>
