...
// ─── Design tokens (matches clinic page) ─────────────────────────────────────
const BG     = '#f8faff'
const SURFACE = '#eef4ff'
const TEXT   = '#0f1829'
const MUTED  = 'rgba(15,24,41,0.48)'
const SUBTLE = 'rgba(15,24,41,0.22)'
const ACCENT = '#0284c7'
const CARD   = 'rgba(255,255,255,0.92)'
const BORDER = 'rgba(2,132,199,0.2)'
...
function NotificationCard({ icon, title, sub, color, opacity, x }: {
  ...
  return (
    <div style={{ opacity, transform: `translateX(${x}px)`, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 4px 24px rgba(2,132,199,0.08)', width: 640 }}>
...
export function ClinicReel() {
  ...
  return (
    <AbsoluteFill style={{ background: `linear-gradient(160deg, ${BG} 0%, ${SURFACE} 100%)`, fontFamily: 'system-ui, -apple-system, sans-serif', opacity: globalOp, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-5%', right: '-5%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(2,132,199,0.07), transparent 60%)', filter: 'blur(80px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '-10%', left: '5%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(2,132,199,0.05), transparent 60%)', filter: 'blur(60px)', pointerEvents: 'none' }} />

      {/* ACT 1 */}
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <div style={{ opacity: badgeOp, marginBottom: 28, display: 'flex', alignItems: 'center', gap: 8, background: `rgba(2,132,199,0.08)`, border: `1px solid ${BORDER}`, borderRadius: 100, padding: '8px 22px' }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: ACCENT }} />
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: ACCENT }}>Pacific Hwy · North Sydney · Est. 2001</span>
        </div>
        <div style={{ opacity: titleOp, transform: `translateY(${titleY}px)`, textAlign: 'center' }}>
          <div style={{ fontSize: 100, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 0.92, color: TEXT }}>Pinnacle</div>
          <div style={{ fontSize: 80, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1, color: ACCENT }}>Health</div>
        </div>
        <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, marginTop: 24 }}>
          <div style={{ fontSize: 22, color: MUTED, textAlign: 'center' }}>Premium private medical · North Sydney</div>
        </div>
        <div style={{ opacity: taglineOp, transform: `translateY(${taglineY}px)`, marginTop: 20 }}>
          <div style={{ fontSize: 24, fontWeight: 500, color: ACCENT, textAlign: 'center', fontStyle: 'italic' }}>
            &ldquo;Where follow-up actually follows up.&rdquo;
          </div>
        </div>
      </AbsoluteFill>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14 }}>
        ...
        <NotificationCard opacity={n1op} x={n1x} icon="📋" title="New patient booked" sub="Sarah M. · Online form → confirmed in 45 sec" color={ACCENT} />
        <NotificationCard opacity={n2op} x={n2x} icon="🔔" title="Recall reminder sent" sub="David T. · Annual check-up due · appointment booked" color="#059669" />
        <NotificationCard opacity={n3op} x={n3x} icon="⭐" title="5-star review received" sub="&ldquo;Most organised practice I&apos;ve ever attended&rdquo; — David T." color="#f59e0b" />
        <NotificationCard opacity={n4op} x={n4x} icon="💬" title="Post-consult follow-up sent" sub="Linda R. · Results ready · secure link delivered" color={ACCENT} />
      </AbsoluteFill>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        ...
        <div style={{ opacity: headOp, transform: `translateY(${headY}px)`, textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontSize: 44, fontWeight: 800, color: TEXT, letterSpacing: '-0.03em', lineHeight: 1.1 }}>North Sydney&apos;s most trusted practice</div>
        </div>
        <div style={{ opacity: quoteOp, transform: `translateY(${quoteY}px)`, textAlign: 'center', maxWidth: 760 }}>
          <div style={{ fontSize: 26, fontWeight: 400, color: TEXT, lineHeight: 1.6, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 24, padding: '32px 40px', boxShadow: '0 8px 40px rgba(2,132,199,0.08)' }}>
            &ldquo;The preventive health reminders have been life-changing.
            Pinnacle caught something early. That&apos;s the kind of care that matters.&rdquo;
          </div>
        </div>
        <div style={{ opacity: attribOp, marginTop: 20, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: ACCENT }}>— Anna K.</div>
          <div style={{ fontSize: 15, color: SUBTLE, marginTop: 4 }}>Preventive Care · 4 years</div>
        </div>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <div style={{ opacity: ctaOp, transform: `scale(${ctaScale})`, textAlign: 'center', marginBottom: 14 }}>
          <div style={{ fontSize: 72, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.0, color: TEXT }}>Healthcare that</div>
          <div style={{ fontSize: 72, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.0, color: ACCENT }}>actually cares.</div>
        </div>
        <div style={{ opacity: subCtaOp, textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontSize: 20, color: MUTED }}>New patients welcome · North Sydney</div>
        </div>
        <div style={{ opacity: cardOp, transform: `translateY(${cardY}px)` }}>
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 24, padding: '28px 36px', minWidth: 500, boxShadow: '0 12px 50px rgba(2,132,199,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L12 22M2 12L22 12"/></svg>
              </div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: TEXT }}>Pinnacle Health · North Sydney</div>
                <div style={{ fontSize: 13, color: MUTED }}>Appointment confirmed ✅</div>
              </div>
            </div>
            {[
              { icon: '📅', label: 'Wednesday 23 Apr · 2:15pm' },
              { icon: '👩‍⚕️', label: 'Dr Chen — General Practice' },
              { icon: '📍', label: 'Level 4, 1 Pacific Hwy, North Sydney' },
            ].map((d, i) => (
            ...
            <div style={{ background: ACCENT, borderRadius: 100, padding: '14px 28px', textAlign: 'center', fontSize: 16, fontWeight: 700, color: '#fff', boxShadow: '0 8px 30px rgba(2,132,199,0.3)' }}>
