...
// ─── Design tokens (matches allied-health page) ───────────────────────────────
const BG     = '#f7fcfa'
const SURFACE = '#edf8f4'
const TEXT   = '#0d1f1a'
const MUTED  = 'rgba(13,31,26,0.48)'
const SUBTLE = 'rgba(13,31,26,0.22)'
const ACCENT = '#0d9488'
const CARD   = 'rgba(255,255,255,0.92)'
const BORDER = 'rgba(13,148,136,0.2)'
...
function NotificationCard({ icon, title, sub, color, opacity, x }: {
  ...
  return (
    <div style={{ opacity, transform: `translateX(${x}px)`, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 4px 24px rgba(13,148,136,0.08)', width: 640 }}>
...
export function AlliedHealthReel() {
  ...
  return (
    <AbsoluteFill style={{ background: `linear-gradient(160deg, ${BG} 0%, ${SURFACE} 100%)`, fontFamily: 'system-ui, -apple-system, sans-serif', opacity: globalOp, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-5%', right: '-5%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(13,148,136,0.08), transparent 60%)', filter: 'blur(80px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '-10%', left: '5%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(13,148,136,0.05), transparent 60%)', filter: 'blur(60px)', pointerEvents: 'none' }} />

      {/* ACT 1 */}
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <div style={{ opacity: badgeOp, marginBottom: 28, display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(13,148,136,0.08)', border: `1px solid ${BORDER}`, borderRadius: 100, padding: '8px 22px' }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: ACCENT }} />
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: ACCENT }}>Crows Nest · Sydney · Est. 2014</span>
        </div>
        <div style={{ opacity: titleOp, transform: `translateY(${titleY}px)`, textAlign: 'center' }}>
          <div style={{ fontSize: 120, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 0.92, color: TEXT }}>Restore</div>
          <div style={{ fontSize: 60, fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1.1, color: ACCENT }}>Allied Health</div>
        </div>
        <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, marginTop: 24 }}>
          <div style={{ fontSize: 22, color: MUTED, textAlign: 'center' }}>Physio · Occupational Therapy · Pilates</div>
        </div>
        <div style={{ opacity: taglineOp, transform: `translateY(${taglineY}px)`, marginTop: 20 }}>
          <div style={{ fontSize: 24, fontWeight: 500, color: ACCENT, textAlign: 'center', fontStyle: 'italic' }}>
            &ldquo;Where recovery actually happens.&rdquo;
          </div>
        </div>
      </AbsoluteFill>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14 }}>
        <div style={{ opacity: labelOp, marginBottom: 6, alignSelf: 'flex-start', marginLeft: 320 }}>
          <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: ACCENT }}>Practice Activity · Today</div>
        </div>
        <NotificationCard opacity={n1op} x={n1x} icon="📋" title="New referral received" sub="Emma T. · GP referral → intake form sent instantly" color={ACCENT} />
        <NotificationCard opacity={n2op} x={n2x} icon="🔔" title="Progress check-in sent" sub="Mark S. · Week 3 · &ldquo;How is your shoulder feeling?&rdquo;" color="#059669" />
        <NotificationCard opacity={n3op} x={n3x} icon="⭐" title="5-star review received" sub="&ldquo;92% goal achievement rate — and I&apos;m one of them.&rdquo;" color="#f59e0b" />
        <NotificationCard opacity={n4op} x={n4x} icon="📅" title="Recall reminder sent" sub="Chen W. · 6-week follow-up · appointment booked" color={ACCENT} />
      </AbsoluteFill>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        ...
        <div style={{ opacity: headOp, transform: `translateY(${headY}px)`, textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontSize: 44, fontWeight: 800, color: TEXT, letterSpacing: '-0.03em', lineHeight: 1.1 }}>Crows Nest&apos;s most trusted practice</div>
        </div>
        <div style={{ opacity: quoteOp, transform: `translateY(${quoteY}px)`, textAlign: 'center', maxWidth: 760 }}>
          <div style={{ fontSize: 26, fontWeight: 400, color: TEXT, lineHeight: 1.6, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 24, padding: '32px 40px', boxShadow: '0 8px 40px rgba(13,148,136,0.08)' }}>
            &ldquo;My PT sessions are tracked, my exercises are logged, I get a weekly check-in.
            No other practice does this. It&apos;s like having a coach 24/7.&rdquo;
          </div>
        </div>
        <div style={{ opacity: attribOp, marginTop: 20, textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: ACCENT }}>— Sarah M.</div>
          <div style={{ fontSize: 15, color: SUBTLE, marginTop: 4 }}>Shoulder rehab · 8 weeks</div>
        </div>
        <div style={{ opacity: googleOp, marginTop: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 17, fontWeight: 600, color: MUTED }}>628 five-star Google reviews</span>
        </div>
      </AbsoluteFill>
      ...
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <div style={{ opacity: ctaOp, transform: `scale(${ctaScale})`, textAlign: 'center', marginBottom: 14 }}>
          <div style={{ fontSize: 72, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.0, color: TEXT }}>Recovery that</div>
          <div style={{ fontSize: 72, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.0, color: ACCENT }}>actually works.</div>
        </div>
        <div style={{ opacity: subCtaOp, textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontSize: 20, color: MUTED }}>New patients welcome · Crows Nest, Sydney</div>
        </div>
        <div style={{ opacity: cardOp, transform: `translateY(${cardY}px)` }}>
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 24, padding: '28px 36px', minWidth: 500, boxShadow: '0 12px 50px rgba(13,148,136,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🌿</div>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: TEXT }}>Restore Allied Health</div>
                <div style={{ fontSize: 13, color: MUTED }}>Session booking confirmed ✅</div>
              </div>
            </div>
            {[
              { icon: '📅', label: 'Friday 25 Apr · 9:30am' },
              { icon: '🦴', label: 'Initial physio assessment · shoulder' },
              { icon: '📍', label: 'Suite 3, 42 Willoughby Rd, Crows Nest' },
            ].map((d, i) => (
            ...
            <div style={{ background: ACCENT, borderRadius: 100, padding: '14px 28px', textAlign: 'center', fontSize: 16, fontWeight: 700, color: '#fff', boxShadow: '0 8px 30px rgba(13,148,136,0.3)' }}>
              Book a session →
            </div>
          </div>
        </div>
